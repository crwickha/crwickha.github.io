---
layout: post
title: "Give Claude Eyes on Your Network: A NetFlow MCP Server for Meraki MX"
date: 2026-03-16
description: "Build a self-hosted NetFlow v9 collector that gives Claude.ai real-time network traffic analysis over Meraki MX flows via the Model Context Protocol."
---
...
Each tool is designed for a specific analysis pattern. Claude chains them together automatically during investigations. The tools are split into focused, fast queries -- each completes in under 2 seconds even against millions of flows, thanks to a tiered pre-aggregation strategy that reduces 50M+ raw flow records to ~70k daily summary rows.
...
The algorithm:
1. Find all src-to-dst pairs with 6+ connections in the time window
